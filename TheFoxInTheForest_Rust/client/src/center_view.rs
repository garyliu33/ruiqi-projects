...
impl CenterView {
    ...
    pub fn draw(&self) {
        ...
        match self.center_card {
            Some(card) => CardView::new(card, self.x + center_view_gap() / 2.0 + card_width() / 2.0, self.y, false).draw(),
            None => EmptyCardView::new(self.x + center_view_gap() / 2.0 + card_width() / 2.0, self.y, false).draw()
        }
    }
}